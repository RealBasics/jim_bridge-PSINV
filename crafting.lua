                ...
                if type(v) == "table" then
                    for l, b in pairs(v) do
                        if crafting and progressBar({
                            ...
                            TriggerEvent('ps-inventory:client:ItemBox', Items[l], "use", b) -- Show item box for each item
                        else
...
function openShop(data)
    ...
    elseif GetResourceState(QBInv):find("start") then
        if QBInvNew then
            ...
            TriggerServerEvent(Config.General.JimShops and "jim-shops:ShopOpen" or "ps-inventory:server:OpenInventory", "shop", data.items.label, data.items)
        end    
    else
        TriggerServerEvent(Config.General.JimShops and "jim-shops:ShopOpen" or "ps-inventory:server:OpenInventory", "shop", data.items.label, data.items)
    end
...
RegisterNetEvent(GetCurrentResourceName()..':server:OpenShopNewQB', function(data)
    ...
    exports['ps-inventory']:OpenShop(source, data)
end)
...
function stashRemoveItem(stashItems, stashName, items) local amount = amount and amount or 1
    ...
    elseif GetResourceState(QBInv):find("start") then
        if QBInvNew then
                 for k, v in pairs(items) do
                     exports['ps-inventory']:RemoveItem(stashName, k, v, false, 'crafting')
